...
// contract FNFTHandler is ERC1155, AccessControl, IFNFTHandler {
contract FNFTHandler is ERC1155, AccessControl {
    ...
    constructor(address provider, address vault) ERC1155("") {
    ...
    function mintBatchRec(address[] calldata recipients, uint[] calldata quantities, uint id, uint newSupply, bytes memory data) external {
        supply[id] += newSupply;
...
//     function burnBatch(address account, uint[] memory ids, uint[] memory amounts) external override onlyRevestController {
    ...
    function getBalance(address account, uint id) external view returns (uint) {
        return balanceOf(account, id);
    }

    function getSupply(uint fnftId) public view  returns (uint) {
